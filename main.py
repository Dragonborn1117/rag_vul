...
"""
    ...
    related_code = ContextReader().run(code, contract_name, function_name)
    logger.info("Related code is finished.")

    audit_result = Auditor(related_code).run()
    summarizations.append(audit_result)
        
    judge_result = Judge(code, summarizations).run()
    logger.info("Judge is finished.")
    print(judge_result)
